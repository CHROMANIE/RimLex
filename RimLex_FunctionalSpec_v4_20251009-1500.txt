# RimLex_FunctionalSpec_v4_20251009-1500.txt

RimLex 機能仕様書（Functional Spec） v4
作成: 2025-10-09 15:00 (JST)
対象: RimWorld 1.5 / 1.6, .NET Framework 4.7.2, Harmony 2.x
実装版: **0.10.0-rc6a**（辞書正規化＋安定化リリース候補）
ドキュメント版: **v4（本書）**
Based on SHA256 (prev v3: RimLex_FunctionalSpec_v3_20251009-0825.txt): **d8eaf0e7c7a66be162f42eb2691d4825828e6adc504115c4c62917584fc72553**
...
* Tooltip: `TooltipHandler.TipRegion (string / TipSignal)`
...
* **画面単位の除外**: `EditWindow_Log`, `Page_ModsConfig`, `Dialog_DebugTables` 既定BL
  ...
  * `IncludedWindows` 指定で **ホワイトリスト方式**へ反転
* **自己参照除外**: RimLex設定画面は常時除外
* **動的スパム抑制**: 短時間に**同型（#化後）**の文字列が閾値回数出たら一定時間ミュート
...
* 現在は 0.x 系（安定前）。実装は **0.10.0-rc6a**。
* 互換を壊さない追加: MINOR（0.10.x）
* バグ修正のみ: PATCH（0.10.x → 0.10.x+1）
* 1.0.0 は仕様凍結後に昇格
* ファイルは常に `_vYYYYMMDD-HHMM` サフィックス

---

## 【16. 将来拡張（Roadmap｜辞書管理フェーズ：v4系）】

* **恒久由来インデックス**：`Dict/Index/en_provenance.tsv`

  * 形状化キー（/n正規化＋数字→#）→ `{mods, first_seen, last_seen, count}` を累積
  * **Exportリセットの対象外**（恒久）
* **MOD別辞書の分割出力**：`Dict/PerMod/<Mod>/strings_ja.tsv` 生成

  * 統合辞書＋由来Index／PerMod収集を参照して切り出し
* **MOD別辞書から統合再構築**：`Dict/strings_ja.tsv` に復元

  * 衝突は `Dict/_conflicts.tsv` にレポート（同英語→異訳）
* **辞書の並べ替え（ソート）**：英語アルファベット順／ByModAlpha
* **陳腐化（orphan）レポート**：IndexやPerModに出ていない、または `last_seen` が古いキー
...
* 文書版を **v4** に昇格（機能は rc6a のまま基線化）。
* 次フェーズ「辞書管理」を**正式な仕様項目**として本書に組み込み。
* 運用上の前提（/n正規化・数字形状化・自己参照除外・アトミックI/O・ホットリロード）を**維持**。
* DoDに「由来Indexの恒久性」「分割/統合・衝突検出・ソート/陳腐化」を追記。
...
1. **恒久由来Index**（`Dict/Index/en_provenance.tsv`）
   ...
   * `key_shape` を主キーに `{mods(set), first_seen(UTC), last_seen(UTC), count}` を累積保存。
   * 収集時に**即時更新**。**Exportリセットで消さない**。
   * 設定UIに「PerMod から再構築」ボタン。

2. **MOD別辞書**

   * **分割出力**：`Dict/PerMod/<Mod>/strings_ja.tsv`（統合辞書＋由来Index/PerMod収集から抽出）。
   * **逆変換**：PerMod群から `Dict/strings_ja.tsv` を再構築。
   * **衝突レポート**：`Dict/_conflicts.tsv` に `(english, mod, ja)` を出力。UI/WARN表示。

3. **品質と整頓**

   * **重複キー検出**（同英語→異訳）をログ＆`_conflicts.tsv`。
   * **辞書ソート**（Alpha / ByModAlpha）出力。
   * **陳腐化レポート**（IndexやPerModに一度も出ていない／`last_seen` 閾値超過）を `Dict/_stale.tsv` に出力。

---

## 【19. DoD（完成定義｜収集安定フェーズの継続要件）】

* ユーザー操作なしで**UI表示に連動**して英語を収集（/n一行）
* 辞書により**即時置換**、/n/\n の差異を吸収
* スライダー等の動的UIで**ループや暴走がない**
* Per-Mod 出力と _All 集約／未訳が**一貫**して再構築可能
* 設定UIから雛形生成・整理が**成功**し、件数トーストが出る
* `Patched:` 6系統、I/Oは**アトミック**、ログ一貫
